package com.example.hikenativeapp.ui.edit_observation;

import android.util.Log;
import androidx.lifecycle.LiveData;
import androidx.lifecycle.MutableLiveData;
import androidx.lifecycle.ViewModel;

import com.example.hikenativeapp.data.local.entity.Observation;
import com.example.hikenativeapp.data.repository.HikeRepository;
import com.example.hikenativeapp.data.repository.ObservationRepository;
import com.example.hikenativeapp.val.ObservationValidation;

import java.util.concurrent.ExecutionException;

public class EditObservationViewModel extends ViewModel {

    private static final String TAG = "EditObservationVM";

    private ObservationRepository repository;
    private final MutableLiveData<Observation> _observation = new MutableLiveData<>();
    private final MutableLiveData<Boolean> _updateSuccess = new MutableLiveData<>();
    private final MutableLiveData<String> _errorMessage = new MutableLiveData<>();
    private final MutableLiveData<Boolean> _isLoading = new MutableLiveData<>();

    public LiveData<Observation> observation = _observation;
    public LiveData<Boolean> updateSuccess = _updateSuccess;
    public LiveData<String> errorMessage = _errorMessage;
    public LiveData<Boolean> isLoading = _isLoading;

    public void setRepository(ObservationRepository repository) {
        this.repository = repository;
    }

    public void loadObservation(int observationId) {
        _isLoading.postValue(true);

        new Thread(() -> {
            try {
                Observation obs = repository.getObservationById(observationId).get();
                _isLoading.postValue(false);

                if (obs != null) {
                    _observation.postValue(obs);
                } else {
                    _errorMessage.postValue("Observation not found");
                }
            } catch (ExecutionException | InterruptedException e) {
                _isLoading.postValue(false);
                _errorMessage.postValue("Error loading observation: " + e.getMessage());
            }
        }).start();
    }

    public void updateObservation(Observation observation) {
        _isLoading.postValue(true);

        new Thread(() -> {
            try {
                // Use updateObservationAndSync to automatically sync to backend
                repository.updateObservationAndSync(observation, new HikeRepository.SyncCallback() {
                    @Override
                    public void onSuccess(String message) {
                        Log.d(TAG, "Sync successful: " + message);
                    }

                    @Override
                    public void onError(String error) {
                        Log.e(TAG, "Sync failed: " + error);
                    }
                }).get();

                _isLoading.postValue(false);
                _updateSuccess.postValue(true);
            } catch (ExecutionException | InterruptedException e) {
                _isLoading.postValue(false);
                _errorMessage.postValue("Error updating observation: " + e.getMessage());
            }
        }).start();
    }

    public boolean validateObservation(String observationText, String observationTime) {
        String error = ObservationValidation.validateObservation(observationText, observationTime);
        if (error != null) {
            _errorMessage.postValue(error);
            return false;
        }
        return true;
    }
}
